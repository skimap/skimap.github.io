...
for filename in os.listdir(merge_directory):
    ...
    if filename.endswith(".gpx"):
        ...
        with open(os.path.join(f"{merge_directory}", f"{filename}.html"), "w", encoding="utf-8") as html_file:
            html_file.write(html_content)

while True:
    for filename in os.listdir(merge_directory):
        if filename.endswith(".html"):
            webbrowser.open_new_tab(f"{merge_directory + filename}")
            if input("Correct track? Y/Enter key") == "Y":
                pass
            else:
                os.remove(f"{merge_directory + filename[:-5]}")
    break